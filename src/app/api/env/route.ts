...
export const GET = withAuth(async (req, { auth }) => {
  ...
  const [data, total] = await Promise.all([
    EnvConfigModel.find(query).sort(sortObj).skip((page - 1) * limit).limit(limit).lean(),
    EnvConfigModel.countDocuments(query),
  ]);
  ...
  if (req.nextUrl.searchParams.get("export") === "excel") {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "EnvConfigs");
    const buf = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });
    return new NextResponse(buf, {
      headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": "attachment; filename=env-configs.xlsx" },
    });
  }
...
export const POST = withAuth(async (req, { auth }) => {
  ...
  const config = await EnvConfigModel.create({ ...body, createdBy: auth.userId });
